...
const windowsEnvFallbackCache = new Map<string, string | undefined>();
...
export function parseEnvironmentVariables(input: string): Record<string, string> {
  ...
  for (const line of input.split(/\r?\n/)) {
    ...
    if (eqIndex > 0) {
      ...
      const envRefMatch = value.match(/^\$\{env:([A-Za-z_][A-Za-z0-9_]*)\}$/);
      if (envRefMatch) {
        value = resolveReferencedEnvironmentVariable(envRefMatch[1]) ?? '';
      }
      if (key) {
        result[key] = value;
      }
...
function resolveReferencedEnvironmentVariable(name: string): string | undefined {
  ...
  if (windowsEnvFallbackCache.has(normalizedName)) {
    return windowsEnvFallbackCache.get(normalizedName);
  }

  const userValue = queryWindowsEnvironmentValue('HKCU\\Environment', normalizedName);
  if (userValue !== undefined && userValue !== '') {
    windowsEnvFallbackCache.set(normalizedName, userValue);
    return userValue;
  }

  const machineValue = queryWindowsEnvironmentValue(
    'HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment',
    normalizedName,
  );
  windowsEnvFallbackCache.set(normalizedName, machineValue);
  return machineValue;
}

function queryWindowsEnvironmentValue(registryPath: string, name: string): string | undefined {
  try {
    const output = execFileSync(
      'reg.exe',
      ['query', registryPath, '/v', name],
      {
        encoding: 'utf8',
    ...
    for (const line of lines) {
      ...
      const match = trimmed.match(new RegExp(`^${escapeRegExp(name)}\\s+REG_\\w+\\s+(.+)$`, 'i'));
      if (match) {
        return match[1].trim();
      }
    }
