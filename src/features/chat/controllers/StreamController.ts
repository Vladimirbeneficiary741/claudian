...
// Keywords indicating provider errors that should trigger an automatic fallback.
...
const CLAUDE_ACCESS_DENIED_PATTERNS = [
  /request not allowed/i,
  /forbidden/i,
  /failed to authenticate\.\s*api error:\s*403/i,
];

function isClaudeAccessDeniedError(message: string): boolean {
  return CLAUDE_ACCESS_DENIED_PATTERNS.some(p => p.test(message));
}
...
export interface StreamControllerDeps {
  ...
  /** Called when a provider quota/auth error is detected so the tab can continue via fallback. */
  onQuotaExhausted?: (request: ProviderFallbackRequest) => Promise<boolean> | boolean;
  /** Called when the user clicks "Re-login" on the auth error banner. */
  onAuthRetry?: (providerId: ProviderId) => void;
}
...
export class StreamController {
  ...
  private static readonly AUTH_RETRY_COOLDOWN_MS = 30000;

  private deps: StreamControllerDeps;
  private lastAutoAuthRetryAt = 0;

  // Provider lifecycle agent tracking (spawn 鈫?wait/close lifecycle)
  private lifecycleSubagentStates = new Map<string, SubagentState>(); // spawn callId 鈫?SubagentState
  private lifecycleAgentIdToSpawnId = new Map<string, string>();      // agentId 鈫?spawn callId
  ...
  async handleStreamChunk(chunk: StreamChunk, msg: ChatMessage): Promise<void> {
    ...
    switch (chunk.type) {
      ...
      case 'error':
        ...
        this.flushPendingTools();
        const normalizedError = isClaudeAccessDeniedError(chunk.content)
          ? 'Claude Code is logged in, but Anthropic rejected this account for Claude Code requests (403 Request not allowed).'
          : chunk.content;
        await this.appendText(`\n\nError: ${normalizedError}`);
        const shouldTriggerFallback = isQuotaError(chunk.content) || isAuthError(chunk.content);
        const fallbackReason = isAuthError(chunk.content) ? 'auth' : 'quota';
        let autoFallbackStarted = false;
        if (shouldTriggerFallback && this.deps.onQuotaExhausted) {
          const pid = this.getActiveProviderId();
          if (
            fallbackReason === 'auth' &&
            pid === 'claude' &&
            !isClaudeAccessDeniedError(chunk.content) &&
            this.deps.onAuthRetry &&
            Date.now() - this.lastAutoAuthRetryAt > StreamController.AUTH_RETRY_COOLDOWN_MS
          ) {
            this.lastAutoAuthRetryAt = Date.now();
            this.deps.onAuthRetry(pid);
          }
          autoFallbackStarted = await Promise.resolve(this.deps.onQuotaExhausted({
            currentProviderId: pid,
            reason: fallbackReason,
            errorMessage: normalizedError,
          }));
          if (!autoFallbackStarted) {
            this.renderQuotaFallbackBanner(msg, pid, fallbackReason, normalizedError);
          }
        }
        break;
  ...
  // ============================================
  // Provider lifecycle subagents (spawn 鈫?wait/close)
  // ============================================
  ...
  private async handleToolResult(
    ...
    if (existingToolCall) {
      ...
      // blocked detection 鈥?their status is determined solely by isError
      if (chunk.isError) {
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.thinkingIndicatorTimeout = setTimeout(() => {
      ...
      const updateTimer = () => {
        ...
        timerSpan.setText(` (esc to interrupt 路 ${formatDurationMmSs(elapsedSeconds)})`);
      };
  ...
  private notifyVaultFileChange(input: Record<string, unknown>): void {
    ...
    setTimeout(() => {
      ...
      if (file instanceof TFile) {
        // Existing file 鈥?tell listeners the content changed
        vault.trigger('modify', file);
      } else {
        // New file 鈥?scan parent directory so Obsidian discovers it
  ...
  /**
   * Renders an inline banner with provider-switch buttons after a provider error.
   * Fallback order: codex -> claude -> deepseek (wraps around).
   */
  private renderQuotaFallbackBanner(
    msg: ChatMessage,
    currentProviderId: ProviderId,
    reason: 'quota' | 'auth',
    errorMessage: string,
    ...
    const others = getOrderedFallbackProviders(currentProviderId, enabled);

    const contentEl = this.deps.state.currentContentEl;
    if (!contentEl) return;

    const banner = document.createElement('div');
    banner.className = 'claudian-quota-fallback-banner';
    const isClaudeAccessDenied = currentProviderId === 'claude' && isClaudeAccessDeniedError(errorMessage);

    const label = document.createElement('span');
    label.className = 'claudian-quota-fallback-label';
    label.textContent = isClaudeAccessDenied
      ? 'Claude Code is logged in, but this account is not currently allowed to use Claude Code requests. Switch provider:'
      : reason === 'auth'
        ? 'Automatic Claude handoff was unavailable. Re-login or switch provider:'
        : 'Automatic provider handoff was unavailable. Switch to:';
    banner.appendChild(label);

    // For auth errors on the current provider, add a Re-login button
    if (reason === 'auth' && this.deps.onAuthRetry && !isClaudeAccessDenied) {
      const reloginBtn = document.createElement('button');
      reloginBtn.className = 'claudian-quota-fallback-btn claudian-relogin-btn';
      reloginBtn.textContent = `Re-login to ${ProviderRegistry.getProviderDisplayName(currentProviderId)}`;
      reloginBtn.addEventListener('click', () => {
        this.deps.onAuthRetry?.(currentProviderId);
        banner.remove();
      });
    ...
    for (const pid of others) {
      const displayName = ProviderRegistry.getProviderDisplayName(pid);
      const btn = document.createElement('button');
      btn.className = 'claudian-quota-fallback-btn';
      btn.textContent = `Switch to ${displayName}`;
      btn.addEventListener('click', () => {
        void this.deps.onQuotaExhausted?.({
          currentProviderId,
