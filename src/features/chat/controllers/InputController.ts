...
export interface InputControllerDeps {
  ...
  applyPendingProviderHandoff?: () => Promise<void> | void;
}
...
export class InputController {
      ...
      if (!ready) {
        if (contentOverride !== undefined) {
          this.restoreMessageToInput({
            ...
            images: imagesForMessage,
            editorContext: options?.editorContextOverride ?? null,
            browserContext: options?.browserContextOverride ?? null,
            canvasContext: options?.canvasContextOverride ?? null,
          });
        }
      ...
      if (!wasInvalidated && state.streamGeneration === streamGeneration) {
        ...
        if (!planApprovalInvalidated) {
          ...
          if (planAutoSendContent) {
            ...
            } else if (state.pendingProviderHandoff) {
              await this.processPendingProviderHandoff();
  ...
  scheduleProviderHandoff(handoffContent: string): void {
    const { state } = this.deps;
    const trimmedHandoff = handoffContent.trim();
    if (!trimmedHandoff) {
      return;
    }

    const queuedFollowUp = this.mergePendingMessages(
      this.pendingSteerMessage,
      state.queuedMessage,
    );

    let content = trimmedHandoff;
    if (queuedFollowUp?.content.trim()) {
      content += `\n\nAdditional queued user follow-up to incorporate after the handoff:\n${queuedFollowUp.content.trim()}`;
    }

    state.pendingProviderHandoff = {
      content,
      images: queuedFollowUp?.images ? [...queuedFollowUp.images] : undefined,
      editorContext: queuedFollowUp?.editorContext ?? null,
      browserContext: queuedFollowUp?.browserContext ?? null,
      canvasContext: queuedFollowUp?.canvasContext ?? null,
    };
    state.queuedMessage = null;
    this.clearPendingSteerState();
    this.updateQueueIndicator();
    ...
    if (!state.isStreaming) {
      setTimeout(() => {
        void this.processPendingProviderHandoff();
      }, 0);
    }
  }
  ...
  private async processPendingProviderHandoff(): Promise<void> {
    const { state, conversationController } = this.deps;
    const handoffMessage = state.pendingProviderHandoff;
    if (!handoffMessage) {
      return;
    }
    ...
    try {
      await conversationController.createNew();
      await this.deps.applyPendingProviderHandoff?.();
      state.pendingProviderHandoff = null;

      const inputEl = this.deps.getInputEl();
      inputEl.value = handoffMessage.content;
      if (handoffMessage.images && handoffMessage.images.length > 0) {
        this.deps.getImageContextManager()?.setImages(handoffMessage.images);
      }

      this.sendMessage({
        content: handoffMessage.content,
        editorContextOverride: handoffMessage.editorContext,
        browserContextOverride: handoffMessage.browserContext ?? null,
        canvasContextOverride: handoffMessage.canvasContext,
      }).catch(() => {
        this.restoreMessageToInput(handoffMessage);
      });
    } catch {
      state.pendingProviderHandoff = handoffMessage;
      this.restoreMessageToInput(handoffMessage);
      this.updateQueueIndicator();
      new Notice('Automatic provider handoff could not continue. Your task was restored to the input.');
    }
  }
