...
import type { ChatMessage, Conversation, ToolCallInfo } from '../../../core/types';
...
import type { ProviderFallbackRequest } from '../controllers/StreamController';
...
type PendingProviderHandoffTarget = {
...
const PROVIDER_FALLBACK_ORDER: ProviderId[] = ['codex', 'claude', 'deepseek'];

function getOrderedFallbackProviders(
  currentProviderId: ProviderId,
  enabledProviderIds: ProviderId[],
): ProviderId[] {
  const ordered = PROVIDER_FALLBACK_ORDER.filter(id => enabledProviderIds.includes(id));
  const extra = enabledProviderIds.filter(id => !ordered.includes(id));
  const merged = [...ordered, ...extra];
  const currentIndex = merged.indexOf(currentProviderId);
  if (currentIndex < 0) {
    return merged;
  }
  return [...merged.slice(currentIndex + 1), ...merged.slice(0, currentIndex)]
    .filter(id => id !== currentProviderId);
}

function collectChangedFilePaths(messages: ChatMessage[]): string[] {
  const seen = new Set<string>();
  const result: string[] = [];

  const visitToolCalls = (toolCalls?: ToolCallInfo[]) => {
    for (const toolCall of toolCalls ?? []) {
      const diffPath = toolCall.diffData?.filePath?.trim();
      const inputPath = typeof toolCall.input.file_path === 'string'
        ? toolCall.input.file_path.trim()
        : null;
      const filePath = diffPath || inputPath;
      if (filePath && !seen.has(filePath)) {
        seen.add(filePath);
        result.push(filePath);
      }

      if (toolCall.subagent?.toolCalls?.length) {
        visitToolCalls(toolCall.subagent.toolCalls);
      }
    }
  };

  for (const message of messages) {
    visitToolCalls(message.toolCalls);
  }

  return result;
}

function getLatestUserTask(messages: ChatMessage[]): string | null {
...
function sanitizeAssistantProgress(content: string): string {
  return content
    .replace(/\n{0,2}Error:\s[\s\S]*$/i, '')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
...
function getLatestAssistantProgress(messages: ChatMessage[]): string | null {
  for (let i = messages.length - 1; i >= 0; i--) {
    const message = messages[i];
    ...
    const content = sanitizeAssistantProgress(message.content || '');
    if (content) {
      return content.length > 1400 ? `${content.slice(0, 1400)}...` : content;
    }
  }
...
function buildProviderHandoffPrompt(
  tab: TabData,
  plugin: ClaudianPlugin,
  nextProvider: ProviderId,
  request: ProviderFallbackRequest,
): string {
  const previousProviderName = ProviderRegistry.getProviderDisplayName(request.currentProviderId);
  const nextProviderName = ProviderRegistry.getProviderDisplayName(nextProvider);
  const latestUserTask = getLatestUserTask(tab.state.messages);
  const latestProgress = getLatestAssistantProgress(tab.state.messages);
  const changedFiles = collectChangedFilePaths(tab.state.messages).slice(0, 12);
  const todos = (tab.state.currentTodos ?? [])
    .map(todo => `- [${todo.status === 'completed' ? 'x' : ' '}] ${todo.content}`)
    .slice(0, 10);
  const currentNote = tab.ui.fileContextManager?.getCurrentNotePath() || null;
  const handoffReason = request.reason === 'auth'
    ? 'the previous provider lost authentication'
    : 'the previous provider hit a usage or quota limit';

  const sections = [
    'Continue the same coding task after an automatic provider handoff.',
    '',
    `Handoff reason: ${handoffReason}.`,
    `Previous provider: ${previousProviderName}.`,
    `Continue now with: ${nextProviderName}.`,
  ];

  if (latestUserTask) {
    sections.push('', 'Original user task:', latestUserTask);
  }

  if (latestProgress) {
    sections.push('', 'Latest progress from the previous provider:', latestProgress);
  }

  if (changedFiles.length > 0) {
    sections.push('', 'Files already changed in the workspace:');
    sections.push(...changedFiles.map(filePath => `- ${filePath}`));
  }

  if (todos.length > 0) {
    sections.push('', 'Outstanding todo state from the interrupted run:');
    sections.push(...todos);
  }

  if (currentNote) {
    sections.push('', `Current note context: ${currentNote}`);
  }

  if (tab.state.planFilePath) {
    sections.push(`Plan file from the interrupted run: ${tab.state.planFilePath}`);
  }

  sections.push(
    '',
    'Please inspect the current repo state, avoid redoing finished work, and continue the task from where it stopped.',
  );
...
async function maybePromptClaudeAuth(
  plugin: ClaudianPlugin,
  providerId: ProviderId,
): Promise<boolean> {
  ...
  try {
    runtime = ProviderRegistry.createChatRuntime({ plugin, providerId: 'claude' }) as ChatRuntime & {
      ensureAuthenticatedInteractive?: () => Promise<boolean>;
      cleanup?: () => void;
    };
    const result = await runtime.ensureAuthenticatedInteractive?.();
    return result ?? true;
  } catch {
    return false;
  } finally {
...
function shouldSuppressAutomaticProviderFallback(
  tab: TabData,
  request: ProviderFallbackRequest,
): boolean {
  if (request.requestedProviderId) {
    return false;
  }

  if (/request not allowed|anthropic rejected this account/i.test(request.errorMessage)) {
    return false;
  }

  return tab.manualProviderSelection === request.currentProviderId;
}

function setManualProviderSelection(
  tab: TabData,
  providerId: ProviderId | null,
): void {
  tab.manualProviderSelection = providerId;
}
...
export async function initializeTabService(
  ...
  try {
    ...
    // Re-check after async operations 鈥?tab may have been closed during init
...
function initializeInputToolbar(
  ...
  const toolbarComponents = createInputToolbar(inputToolbar, {
    ...
    getCurrentProviderLabel: () => ProviderRegistry.getProviderDisplayName(getTabProviderId(tab, plugin)),
    getSettings: () => getTabSettingsSnapshot(tab, plugin),
    getEnvironmentVariables: () => plugin.getActiveEnvironmentVariables(getTabProviderId(tab, plugin)),
    // Always expose all-provider model list so users can switch providers directly in the toolbar
    getAllModelOptions: () => getBlankTabModelOptions(plugin.settings as unknown as Record<string, unknown>),
    onModelChange: async (model: string) => {
      ...
      if (tab.lifecycleState === 'blank') {
        ...
        setManualProviderSelection(tab, newProvider);
        if (tab.service) {
        ...
        tab.ui.modelSelector?.renderOptions();
        applyProviderUIGating(tab, plugin);
        void maybePromptClaudeAuth(plugin, newProvider);
        return;
      }

      // For bound tabs: allow cross-provider switches (quota fallback scenario)
      ...
      setManualProviderSelection(tab, modelProvider);
      if (modelProvider !== boundProvider) {
        // Switch the tab's provider 鈥?reset session so new provider starts fresh
        cleanupTabRuntime(tab);
        ...
        syncTabProviderServices(tab, plugin);
        syncSlashCommandDropdownForProvider(tab, plugin, getProviderCatalogConfig);
        onProviderChanged?.(modelProvider);
        const uiConfig2 = ProviderRegistry.getChatUIConfig(modelProvider);
        await updateTabProviderSettings(tab, plugin, (settings) => {
          settings.model = model;
          uiConfig2.applyModelDefaults(model, settings);
        });
        refreshTabProviderUI(tab, plugin);
        applyProviderUIGating(tab, plugin);
        void maybePromptClaudeAuth(plugin, modelProvider);
        return;
      }
...
/** @deprecated Legacy 7-arg overload 鈥?4th arg was previously an MCP manager. */
export function initializeTabControllers(
  tab: TabData,
  plugin: ClaudianPlugin,
  component: Component,
...
export function initializeTabControllers(
  ...
  let pendingProviderHandoffTarget: PendingProviderHandoffTarget | null = null;
  ...
  tab.controllers.streamController = new StreamController({
    ...
    // When auth error occurs, trigger re-login on the current provider runtime (Claude only).
    onAuthRetry: (providerId) => {
      const runtime = tab.service;
      if (!runtime) return;
      const authRuntime = runtime as unknown as {
        ensureAuthenticatedInteractive?: () => Promise<boolean>;
        triggerRelogin?: () => Promise<void>;
      };
      if (typeof authRuntime.ensureAuthenticatedInteractive === 'function') {
        void authRuntime.ensureAuthenticatedInteractive();
        return;
      }
      if (typeof authRuntime.triggerRelogin === 'function') {
        void authRuntime.triggerRelogin();
      }
    },
    // When a provider quota/auth error is detected, switch the tab and continue automatically.
    onQuotaExhausted: (request) => {
      if (shouldSuppressAutomaticProviderFallback(tab, request)) {
        new Notice(
          `Staying on ${ProviderRegistry.getProviderDisplayName(request.currentProviderId)} because you selected it manually. Use the switch button if you want to hand off.`,
        );
        return false;
      }

      const settings = plugin.settings as unknown as Record<string, unknown>;
      const enabled: ProviderId[] = ProviderRegistry.getEnabledProviderIds(settings);
      const fallbackCandidates = getOrderedFallbackProviders(request.currentProviderId, enabled);
      const nextProvider = (
        request.requestedProviderId
        && request.requestedProviderId !== request.currentProviderId
        && enabled.includes(request.requestedProviderId)
      )
        ? request.requestedProviderId
        : (fallbackCandidates[0] ?? request.currentProviderId);
      if (nextProvider === request.currentProviderId) {
        return false;
      }

      const nextModels = ProviderRegistry.getChatUIConfig(nextProvider).getModelOptions(settings);
      const nextModel: string = nextModels[0]?.value;
      if (!nextModel) {
        return false;
      }

      const handoffPrompt = buildProviderHandoffPrompt(tab, plugin, nextProvider, request);
      const inputController = tab.controllers.inputController;
      if (!inputController) {
        return false;
      }

      pendingProviderHandoffTarget = {
        providerId: nextProvider,
        model: nextModel,
        explicit: Boolean(request.requestedProviderId),
      };
      inputController.scheduleProviderHandoff(handoffPrompt);
      new Notice(`Preparing automatic handoff to ${ProviderRegistry.getProviderDisplayName(nextProvider)}...`);
      return true;
    },
  });
  ...
  tab.controllers.conversationController = new ConversationController(
    {
      ...
      ensureServiceForConversation: async (conversation) => {
        ...
        setManualProviderSelection(tab, null);

        if (providerChanged) {
          syncTabProviderServices(tab, plugin);
        }

        // Bind session state only 鈥?runtime starts on send
  ...
  tab.controllers.inputController = new InputController({
    ...
    applyPendingProviderHandoff: async () => {
      const target = pendingProviderHandoffTarget;
      if (!target) {
        return;
      }

      if (!(await maybePromptClaudeAuth(plugin, target.providerId))) {
        throw new Error('Claude authentication is still required.');
      }

      pendingProviderHandoffTarget = null;
      cleanupTabRuntime(tab);
      tab.providerId = target.providerId;
      setManualProviderSelection(tab, target.explicit ? target.providerId : null);
      tab.conversationId = null;
      tab.state.currentConversationId = null;
      tab.lifecycleState = 'blank';
      tab.draftModel = target.model;
      syncTabProviderServices(tab, plugin);
      const uiCfg = ProviderRegistry.getChatUIConfig(target.providerId);
      await updateTabProviderSettings(tab, plugin, (s: Record<string, unknown>) => {
        s.model = target.model;
        uiCfg.applyModelDefaults(target.model, s);
      });
      syncSlashCommandDropdownForProvider(tab, plugin, getProviderCatalogConfig);
      refreshTabProviderUI(tab, plugin);
      applyProviderUIGating(tab, plugin);
...
export function wireTabInputEvents(tab: TabData, plugin: ClaudianPlugin): void {
  ...
  // Sidebar focus handler 鈥?show selection highlight when focus enters the tab from outside
    ...
    tab.service.setExitPlanModeCallback(
      async (input, signal) => {
        ...
        if (decision !== null && decision.type !== 'feedback') {
          // Only restore permission mode if still in plan mode 鈥?user may have toggled out via Shift+Tab
          if (plugin.settings.permissionMode === 'plan') {
            const restoreMode = tab.state.prePlanPermissionMode ?? 'normal';
            tab.state.prePlanPermissionMode = null;
            updatePlanModeUI(tab, plugin, restoreMode);
