...
export interface ToolbarCallbacks {
  ...
  /** When provided, lists models from ALL enabled providers (cross-provider switching). */
  getAllModelOptions?: () => ProviderUIOption[];
}
...
export class ModelSelector {
  ...
  private getAvailableModels() {
    // Prefer cross-provider list when callback is provided (bound conversation tabs)
    if (this.callbacks.getAllModelOptions) {
      return this.callbacks.getAllModelOptions();
    }
  ...
  updateDisplay() {
    ...
    const providerLabel = this.callbacks.getCurrentProviderLabel?.()?.trim();
    ...
    if (providerLabel) {
      this.buttonEl.createSpan({
        cls: 'claudian-model-provider-label',
        text: `${providerLabel} / `,
      });
    }
