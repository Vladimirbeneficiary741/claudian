...
export const claudeSettingsTabRenderer: ProviderSettingsTabRenderer = {
  render(container, context) {
    ...
    // Returns { message, isHardError } — hard errors block saving, warnings just inform.
    const validatePath = (value: string): { message: string; isHardError: boolean } | null => {
      ...
      try {
        fileExists = fs.existsSync(expandedPath);
      } catch {
        // fs access failed — treat as soft warning, don't block saving
      }

      if (!fileExists) {
        // Soft warning: path may exist but be inaccessible in this environment
        return { message: t('settings.cliPath.validation.notExist'), isHardError: false };
      }

      try {
        const stat = fs.statSync(expandedPath);
        if (!stat.isFile()) {
          return { message: t('settings.cliPath.validation.isDirectory'), isHardError: true };
        }
      } catch {
        // stat failed — allow save
      }

      return null;
    };

    // Returns true if path can be saved (no hard error), shows message for warnings.
    const updateCliPathValidation = (value: string, inputEl?: HTMLInputElement): boolean => {
      const result = validatePath(value);
      if (result) {
        validationEl.setText(result.message);
        validationEl.style.color = result.isHardError ? 'var(--text-error)' : 'var(--text-warning, var(--color-yellow))';
        validationEl.style.display = 'block';
        if (inputEl) {
          inputEl.style.borderColor = result.isHardError ? 'var(--text-error)' : '';
        }
        return !result.isHardError; // warnings still allow saving
      }
