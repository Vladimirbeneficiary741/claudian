...
function normalizeClaudeErrorText(content: string): string {
  const normalized = content
    .replace(/^authentication_failed\s*/i, '')
    .replace(/\s+/g, ' ')
    .trim();

  if (/request not allowed/i.test(normalized) || /forbidden/i.test(normalized)) {
    return 'Claude Code is logged in, but Anthropic rejected this account for Claude Code requests (403 Request not allowed).';
  }
...
export function* transformSDKMessage(
  ...
  switch (message.type) {
    ...
    case 'assistant': {
      ...
      if (message.error) {
        yield { type: 'error', content: normalizeClaudeErrorText(message.error) };
      }
    ...
    case 'result':
      if (isResultError(message)) {
        const content = normalizeClaudeErrorText(
          message.errors.filter((e) => e.trim().length > 0).join('\n')
        );
        yield {
          type: 'error',
