...
/**
 * Returns true if the session file exists and contains at least one `result` message.
 *
 * An incomplete session (no `result`) causes the CLI to exit code=1 or send
 * malformed history to the API → 403 "Request not allowed". Skip resume in that case.
 */
export async function isSessionResumable(vaultPath: string, sessionId: string): Promise<boolean> {
  try {
    const sessionPath = getSDKSessionPath(vaultPath, sessionId);
