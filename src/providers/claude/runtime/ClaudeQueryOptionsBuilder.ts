...
import { createCustomSpawnFunction, stripHostManagedAuthEnv } from './customSpawn';
import {
...
export class QueryOptionsBuilder {
  ...
  private static buildBaseOptions(
    ...
    // Filter undefined values from process.env — Electron renderer exposes some
    // env keys with undefined values, which corrupt the Windows environment block
    // passed to CreateProcessW, causing spawn to fail with ENOENT.
    // Also strip host-managed auth vars (CLAUDE_CODE_OAUTH_TOKEN etc.) inherited
    // from a Claude Desktop parent process — otherwise the CLI uses the stale
    // host token instead of the credentials file and returns 401.
    const baseEnv = stripHostManagedAuthEnv(
      Object.fromEntries(
