...
import { getEnhancedPath, getHostnameKey, getMissingNodeError, parseEnvironmentVariables } from '../../../utils/env';
...
import { findClaudeCLICandidates } from '../cli/findClaudeCLIPath';
import { loadSubagentFinalResult, loadSubagentToolCalls } from '../history/ClaudeHistoryStore';
import { isSessionResumable } from '../history/sdkSessionPaths';
...
import {
  launchClaudeCliDetached,
  launchClaudeCliInConsoleWindow,
  runClaudeCliCommand,
  stripHostManagedAuthEnv,
} from './customSpawn';
import {
...
export class ClaudianService implements ChatRuntime {
  ...
  private lastClaudeAuthLaunchAt = 0;
  ...
  private buildClaudeCliEnv(cliPath: string): {
    ...
    // Strip CLAUDE_CODE_OAUTH_TOKEN and other host-managed auth env vars inherited
    // from the Claude Desktop parent process. See stripHostManagedAuthEnv for why.
    const cleanedProcessEnv = stripHostManagedAuthEnv(process.env as Record<string, string | undefined>);
    return {
      customEnv,
      enhancedPath,
      env: {
  ...
  private async probeClaudeAuthStatus(
    vaultPath: string,
    cliPath: string,
  ): Promise<{
    cliPath: string;
    ...
    const { enhancedPath, env } = this.buildClaudeCliEnv(cliPath);
    ...
    try {
      ...
      return { cliPath, result, loggedIn };
    } catch {
      return null;
    }
  }

  private async resolveWorkingClaudeCliPath(
    vaultPath: string,
    ...
    const candidates = [
      initialCliPath,
      ...findClaudeCLICandidates(customEnv.PATH),
    ].filter((candidate, index, all) =>
      !!candidate && all.findIndex(entry => entry.toLowerCase() === candidate.toLowerCase()) === index,
    );

    for (const candidate of candidates) {
      const probe = await this.probeClaudeAuthStatus(vaultPath, candidate);
      if (!probe) {
        continue;
      }

      if (candidate !== initialCliPath) {
        try {
          updateClaudeProviderSettings(
            this.plugin.settings as unknown as Record<string, unknown>,
            {
              cliPathsByHost: {
                ...getClaudeProviderSettings(this.plugin.settings as unknown as Record<string, unknown>).cliPathsByHost,
                [getHostnameKey()]: candidate,
              },
            },
          );
          void this.plugin.saveSettings();
        } catch {
          // Non-fatal: runtime can still continue with the detected path
        }
      }
  ...
  private async ensureClaudeAuthenticated(
    vaultPath: string,
    cliPath: string,
    options?: { forceRelogin?: boolean },
  ): Promise<boolean> {
    const forceRelogin = options?.forceRelogin === true;
    const resolved = await this.resolveWorkingClaudeCliPath(vaultPath, cliPath);
    const effectiveCliPath = resolved?.cliPath ?? cliPath;
    if (resolved?.loggedIn && !forceRelogin) {
      return true;
    }

    const { enhancedPath, env } = this.buildClaudeCliEnv(effectiveCliPath);

    const now = Date.now();
    if (now - this.lastClaudeAuthLaunchAt > 30000) {
      this.lastClaudeAuthLaunchAt = now;
      try {
        await launchClaudeCliInConsoleWindow(
          effectiveCliPath,
          ['auth', 'login', '--claudeai'],
          {
            cwd: vaultPath,
        ...
        new Notice(
          forceRelogin
            ? 'Claude Code re-login window opened. Complete the CLI-guided browser authorization, then resend your message.'
            : 'Claude Code login window opened. Complete the CLI-guided browser authorization, then resend your message.',
          12000,
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Unknown Claude login error';
        new Notice(`Unable to launch Claude Code sign-in: ${message}`, 12000);
      }
    }
  ...
  /**
   * Forces the Claude Code login window to open, bypassing the local auth status check.
   * Use this when the API returns 401 even though local credentials appear valid.
   */
  async triggerRelogin(): Promise<void> {
    ...
    if (!cliPath) {
      new Notice('Claude Code executable not found. Please configure the CLI path in settings.', 10000);
      return;
    }
    // Force lastClaudeAuthLaunchAt to 0 so the cooldown doesn't block the re-launch
    this.lastClaudeAuthLaunchAt = 0;
    await this.ensureClaudeAuthenticated(vaultPath, cliPath, { forceRelogin: true });
  }

  async ensureAuthenticatedInteractive(): Promise<boolean> {
    const vaultPath = getVaultPath(this.plugin.app) ?? process.cwd();
    const cliPath = this.plugin.getResolvedProviderCliPath('claude');
    if (!cliPath) {
      new Notice('Claude Code executable not found. Please configure the CLI path in settings.', 10000);
    ...
    return this.ensureClaudeAuthenticated(vaultPath, cliPath);
  }
  ...
  async ensureReady(options?: ChatRuntimeEnsureReadyOptions): Promise<boolean> {
    ...
    if (!this.persistentQuery) {
      ...
      if (!(await this.ensureClaudeAuthenticated(vaultPath, cliPath))) return false;
  ...
  private async startPersistentQuery(
    ...
    // Validate resume session before attempting --resume.
    // An incomplete session file (no 'result' message) causes the CLI to exit
    // code=1 or send malformed history to the API, which returns 403
    // "Request not allowed". Skip resume and start fresh in that case.
    const validatedSessionId = resumeSessionId && (await isSessionResumable(vaultPath, resumeSessionId))
      ? resumeSessionId
      : undefined;
    ...
    if (validatedSessionId) {
      this.messageChannel.setSessionId(validatedSessionId);
      this.sessionManager.setSessionId(validatedSessionId, this.getScopedSettings().model);
    }
  ...
  async *query(
    ...
    if (!(await this.ensureClaudeAuthenticated(vaultPath, resolvedClaudePath))) {
      yield {
        type: 'error',
        content: 'Claude Code sign-in is required. The login window has been opened automatically. Finish login, then resend your message.',
      };
      return;
    }
    ...
    if (shouldUsePersistent) {
      ...
      if (!this.persistentQuery && !this.shuttingDown) {
        // Pass externalContextPaths so the first spawn already has --add-dir
        // args matching the current turn. Otherwise applyClaudeDynamicUpdates()
        // would detect a config mismatch and force a close+restart mid-SessionStart,
        // which manifests as "CLI killed ~3s after spawn" in the spawn log and
        // aborts the user's first message.
  ...
  private handleRecoverableSessionChunk(chunk: StreamChunk): never | void {
    if (chunk.type !== 'error') {
      return;
    }
    ...
    this.sessionManager.invalidateSession();
    this.closePersistentQuery('session not found');
    throw error;
  }
    ...
    try {
      ...
      while (!state.done) {
        if (state.chunks.length > 0) {
          ...
          this.handleRecoverableSessionChunk(chunk);
          yield chunk;
        } else {
      ...
      for await (const message of response) {
        ...
        for (const event of transformSDKMessage(message, this.getTransformOptions(selectedModel))) {
          if (isSessionInitEvent(event)) {
            ...
            this.handleRecoverableSessionChunk(event);
