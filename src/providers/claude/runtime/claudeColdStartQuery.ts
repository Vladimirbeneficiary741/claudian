...
export async function runColdStartQuery(
  ...
  const options: Options = {
    ...
    env: {
      // Strip CLAUDE_CODE_OAUTH_TOKEN etc. inherited from a Claude Desktop parent
      // so the CLI uses the credentials file rather than a stale host token.
      ...stripHostManagedAuthEnv(process.env as Record<string, string | undefined>),
      ...customEnv,
      PATH: enhancedPath,
    },
