...
function buildLaunchEnvironmentFingerprint(env: Record<string, string>): string {
  return Object.entries(env)
    .filter(([key]) => /^OPENAI_/i.test(key) || /^CODEX_/i.test(key))
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([key, value]) => {
      if (/(KEY|TOKEN|SECRET)/i.test(key)) {
        return `${key}=present:${value ? value.length : 0}`;
      }
      return `${key}=${value}`;
    })
    .join('|');
...
export class CodexChatRuntime implements ChatRuntime {
  ...
  async ensureReady(options?: ChatRuntimeEnsureReadyOptions): Promise<boolean> {
    ...
    const clientConfigKey = [promptKey, JSON.stringify({
      ...
      environment: buildLaunchEnvironmentFingerprint(launchSpec.env),
    })].join('::');
    ...
    try {
      ...
      if (this.pendingFork) {
        ...
        await this.transport!.request<ThreadResumeResult>('thread/resume', {
          ...
          serviceTier: resolveCodexServiceTier(this.getProviderSettings().serviceTier, model ?? this.defaultModel),
      ...
      if (turn.isCompact) {
        ...
        const summary = getCodexProviderSettings(providerSettings, this.providerId).reasoningSummary;
  ...
  private resolveSandboxConfig(): { approvalPolicy: string; sandbox: string } {
    ...
    return resolveCodexSandboxConfig(
      ...
      getCodexProviderSettings(providerSettings, this.providerId).safeMode,
    );
  }
