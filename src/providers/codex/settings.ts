...
export interface CodexProviderSettings {
  ...
  deepseekBackendEnabled: boolean;
  deepseekModel: string;
}

function normalizeProviderId(providerId: string | undefined): string {
  return typeof providerId === 'string' && providerId.trim() ? providerId.trim() : 'codex';
}
...
export function getCodexProviderSettings(
  ...
  const config = getProviderConfig(settings, normalizedProviderId);
  ...
  return {
    ...
    environmentVariables: (config.environmentVariables as string | undefined)
      ?? getProviderEnvironmentVariables(settings, normalizedProviderId)
    ...
    wslDistroOverridesByHost,
    deepseekBackendEnabled: (config.deepseekBackendEnabled as boolean | undefined)
      ?? DEFAULT_CODEX_PROVIDER_SETTINGS.deepseekBackendEnabled,
    deepseekModel: (config.deepseekModel as string | undefined)
      ?? DEFAULT_CODEX_PROVIDER_SETTINGS.deepseekModel,
  };
}

export function getCodexDeepSeekBackendEnvText(
  settings: Record<string, unknown>,
  providerId = 'codex',
): string {
  const codexSettings = getCodexProviderSettings(settings, providerId);
  if (!codexSettings.deepseekBackendEnabled) {
    return '';
  }

  const model = codexSettings.deepseekModel.trim() || 'deepseek-chat';
  return [
...
export function updateCodexProviderSettings(
  ...
  const normalizedProviderId = normalizeProviderId(providerId);
  const current = getCodexProviderSettings(settings, normalizedProviderId);
  const hostnameKey = getHostnameKey();
  ...
  setProviderConfig(settings, normalizedProviderId, {
    ...
    installationMethodsByHost,
    wslDistroOverridesByHost,
    deepseekBackendEnabled: next.deepseekBackendEnabled,
