...
const DEEPSEEK_MODELS_FOR_CODEX = [
...
export const codexSettingsTabRenderer: ProviderSettingsTabRenderer = {
  render(container, context) {
    ...
    new Setting(container).setName('DeepSeek Backend').setHeading();

    const deepseekBackendDesc = container.createDiv({ cls: 'claudian-sp-settings-desc' });
    deepseekBackendDesc.createEl('p', {
      cls: 'setting-item-description',
      text: 'Use the DeepSeek API instead of OpenAI when running Codex. When enabled, Claudian injects DeepSeek-compatible OPENAI_* variables at runtime and reads the API key from the DEEPSEEK_API_KEY environment variable.',
    });

    let deepseekBackendEnabled = codexSettings.deepseekBackendEnabled;
    let deepseekModelDropdownEl: HTMLElement | null = null;

    const refreshDeepSeekBackendUI = (): void => {
      if (deepseekModelDropdownEl) {
        (deepseekModelDropdownEl as HTMLSelectElement).disabled = !deepseekBackendEnabled;
      }
    };

    new Setting(container)
      .setName('Use DeepSeek API')
      .setDesc('When enabled, Codex connects to DeepSeek\'s API endpoint using DEEPSEEK_API_KEY from your environment.')
      .addToggle((toggle) =>
        toggle
          .setValue(deepseekBackendEnabled)
          .onChange(async (value) => {
            deepseekBackendEnabled = value;
            updateCodexProviderSettings(settingsBag, { deepseekBackendEnabled: value });
            await context.plugin.saveSettings();
            refreshDeepSeekBackendUI();
            context.refreshModelSelectors();
          })
      );
    ...
    new Setting(container)
      ...
      .setDesc('Model to use when DeepSeek backend is enabled. Sets OPENAI_MODEL automatically.')
      .addDropdown((dropdown) => {
        for (const opt of DEEPSEEK_MODELS_FOR_CODEX) {
          dropdown.addOption(opt.value, opt.label);
        }
        dropdown.setValue(codexSettings.deepseekModel || 'deepseek-chat');
        dropdown.onChange(async (value) => {
          updateCodexProviderSettings(settingsBag, { deepseekModel: value });
          await context.plugin.saveSettings();
        });
        (dropdown.selectEl as HTMLSelectElement).disabled = !deepseekBackendEnabled;
        deepseekModelDropdownEl = dropdown.selectEl;
      });
